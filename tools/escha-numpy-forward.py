...
def rope(x, pos):
    # x [..., 256], rotate first 64 dims; NEOX half-split pairing (vendor traditional=False)
    xf = x.copy()
    ang = pos * inv_freq
    cos, sin = np.cos(ang), np.sin(ang)
    a = xf[..., :NROT // 2].copy()
    b = xf[..., NROT // 2:NROT].copy()
    xf[..., :NROT // 2] = a * cos - b * sin
    xf[..., NROT // 2:NROT] = a * sin + b * cos
    return xf
...
def attn_layer(li, h, t, kv):
    ...
    acc('attn_gated', og)
    return escha_mm(pre + '.o_proj', og.reshape(-1))
